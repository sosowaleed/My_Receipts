...
class DatabaseService {
  ...
  Future _onUpgrade(sql.Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 5) {
      // Create the new simulation tables for existing users
      await db.execute('''
        CREATE TABLE simulations (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          profileId INTEGER NOT NULL,
  ...
  */
  ...
  // --- COMPLETE CRUD METHODS FOR SIMULATIONS ---

  /// Creates a new simulation entry in the database.
  Future<Sim> createSimulation({required int profileId, required String name}) async {
    final db = await instance.database;
    final sim = Sim(
    ...
    // We remove the 'id' from the map because it's autoincremented.
    final Map<String, dynamic> simMap = sim.toMap()..remove('id');
    final id = await db.insert('simulations', simMap);
    return Sim(id: id, profileId: profileId, name: name, createdAt: sim.createdAt);
  }

  /// Reads all saved simulations for a given profile.
  Future<List<Sim>> readSimulations(int profileId) async {
    final db = await instance.database;
    final result = await db.query(
      'simulations',
      where: 'profileId = ?',
    ...
    return result.map((json) => Sim.fromMap(json)).toList();
  }

  /// Updates the name of an existing simulation.
  Future<int> updateSimulationName(int simId, String newName) async {
    final db = await instance.database;
  ...
  /// Deletes a simulation and all its associated transactions.
  Future<int> deleteSimulation(int simId) async {
    final db = await instance.database;
    // The 'ON DELETE CASCADE' constraint on the simulated_transactions table
    // will automatically delete all its transactions when the simulation is deleted.
  ...
  /// Creates a copy of all real transactions for a profile into a new simulation.
  Future<void> copyRealTransactionsToSimulation(int simId, int profileId) async {
    final db = await instance.database;
    final realTransactions = await db.query('transactions', where: 'profileId = ?', whereArgs: [profileId]);
  ...
  // --- CRUD for individual simulated transactions ---
  Future<Transaction> createSimulatedTransaction(Transaction tx, int simId) async {
    final db = await instance.database;
    final map = tx.toMap();
    map.remove('id');
  ...
  Future<int> deleteSimulatedTransaction(int txId) async {
    final db = await instance.database;
    return db.delete('simulated_transactions', where: 'id = ?', whereArgs: [txId]);
  }
}
